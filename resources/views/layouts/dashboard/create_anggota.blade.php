...
@section('content')
    <div class="card" style="background: white; padding: 20px; border-radius: 8px;">
        <h3>Form Tambah Anggota Asrama</h3>
        ...
        <form action="{{ route('dashboard.anggota.store') }}" method="POST">
            ...
            <div style="margin-bottom: 15px;">
                ...
                <input type="email" name="email" id="email" value="{{ old('email') }}" placeholder="Masukkan alamat email"
                    style="width: 100%; padding: 8px; margin-top: 5px; border: 1px solid #ccc; border-radius: 4px;" required>
            </div>
            ...
            <div style="margin-bottom: 15px;">
                ...
                <select name="status" id="status" style="width: 100%; padding: 8px; margin-top: 5px; border: 1px solid #ccc; border-radius: 4px;" required>
                    ...
                    <option value="inactive" {{ old('status') == 'inactive' ? 'selected' : '' }}>Non-Aktif (Inactive)</option>
                </select>
            </div>

            <div style="margin-top: 20px;">
                <button type="submit"
                    style="background-color: #2ecc71; color: white; padding: 10px 15px; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">Simpan Anggota</button>
